...
if (isset($_POST['log_submit'])) {
    ...
    $log_pass = base64_encode(
        mysqli_real_escape_string($conn, $_POST['log_pass'])
    );
...
if (isset($_POST['reg_submit'])) {
    ...
    $re_pass = base64_encode(
        mysqli_real_escape_string($conn, $_POST['reg_pass'])
    );

    if ($reg_user == 'admin') {
        header(
            'Location: index.php?re_error=Invalid username you cannot use the username as admin'
        );
    } elseif (
        mysqli_num_rows(
            mysqli_query(
                $conn,
                "SELECT * FROM user WHERE name='$reg_user' OR email='$re_mail'"
            )
        ) > 0
    ) {
        header(
            'Location: index.php?re_error=Username or email id already exists!'
        );
    } else {
        ...
        $run_reg = mysqli_query(
            $conn,
            "INSERT INTO user(name, email, password, number, address, date) VALUES('{$_SESSION['name']}','{$_SESSION['mail']}','{$_SESSION['pass']}', '{$_SESSION['pno']}','{$_SESSION['address']}',NOW() )"
        );
        if ($run_reg) {
            $forid = mysqli_fetch_assoc(
                mysqli_query(
                    $conn,
                    "SELECT * FROM user WHERE name='{$_SESSION['name']}' OR email='{$_SESSION['mail']}'"
                )
            );
...
if (isset($_POST['chg_submit'])) {
    $id = $_SESSION['id'];
    $oldpass = base64_encode(
        mysqli_real_escape_string($conn, $_POST['log_pass'])
    );
    $chkpass = mysqli_fetch_assoc(
        mysqli_query($conn, "SELECT * FROM user WHERE user_id = $id")
    );
    ...
    if ($chkpass['password'] == $oldpass) {
        if ($newpass == $con_pass) {
            ...
            $upadpass = mysqli_query(
                $conn,
                "UPDATE user SET password= '$pass' WHERE user_id = $id"
            );
            if ($upadpass) {
                header(
                    'Location: chgpass.php?success_chg=Password Successfully Changed!'
                );
            }
        } else {
            header(
                'Location: chgpass.php?error_chg=Password mismatched for new and current password!'
            );
        }
    } else {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (
                ...
                $add_sql = mysqli_query(
                    $conn,
                    "INSERT INTO product(p_name, p_img, p_category, p_qty, p_measure, p_price, available) VALUES('$p_name', '{$_SESSION['img']}', '$category', '$qty', '$unit', '$p_price', '$p_avail')"
                );
                if ($add_sql) {
...
if (isset($_POST['d_id'])) {
    $del_id = $_POST['d_id'];
    $del_sql = mysqli_query(
        $conn,
        "DELETE FROM product WHERE p_id = '$del_id'"
    );
    if ($del_sql) {
...
if (isset($_POST['edit'])) {
    ...
    $edit_sql = mysqli_query(
        $conn,
        "UPDATE product SET p_name = '$pe_name', p_price = '$ep_price', available = '$ep_avail' WHERE p_id = '$edit'"
    );
    if ($edit_sql) {
...
if (isset($_POST['search'])) {
    ...
    if (mysqli_num_rows($search_sql) > 0) {
        ...
        $outputs .=
            '<p class="py-4 text-danger h2 text-center">No Item Found!</p>';
    }
    $outputs .= '</div>
                </div>';
    echo $outputs;
...
if (isset($_POST['cart_id'])) {
    $id = $_POST['cart_id'];
    $row10 = mysqli_fetch_assoc(
        mysqli_query($conn, "SELECT * FROM product WHERE p_id ='$id'")
    );
    $item_array = [
        'item_id' => $row10['p_id'],
...
if (isset($_POST['checkout'])) {
    foreach ($_SESSION['shopping_cart'] as $keys => $values) {
        $id = $values['item_id'];
        $row11 = mysqli_fetch_assoc(
            mysqli_query($conn, "SELECT * FROM product WHERE p_id = '$id'")
        );
        $avail = $row11['available'] - $values['item_quantity'];
        mysqli_query(
            $conn,
            "UPDATE product SET available = '$avail' WHERE p_id = '$id'"
        );
    }
}
...
if (isset($_POST['store'])) {
    if (!empty($_SESSION['shopping_cart'])) {
        ...
        mysqli_query(
            $conn,
            "INSERT INTO history(user_id, items, total, date) VALUES('{$_SESSION['id']}', '$item', '$total', NOW())"
        );
    }
}
...
if (isset($_POST['action']) && $_POST['action'] == 'delete') {
    foreach ($_SESSION['shopping_cart'] as $keys => $values) {
        if ($values['item_id'] == $_POST['id']) {
            echo $_SESSION['shopping_cart'][$keys]['item_name'] .
                ' is removed!';
    ...
    if (!empty($_SESSION['shopping_cart'])) {
        ...
        foreach ($_SESSION['shopping_cart'] as $keys => $values) {
            $output .=
                ...
                $values['item_name'] .
                '</td>
                                <td><i class="fa fa-plus-square text-primary btn" onclick="plus(' .
                $values['item_id'] .
                ')"></i><span id=' .
                $values['item_id'] .
                '>' .
                $values['item_quantity'] .
                '</span><i onclick="minus(' .
                $values['item_id'] .
                ')" class="fa fa-minus-square text-primary btn"></i></a>' .
                $values['item_measure'] .
                '</td>
                                <td>₹ ' .
                $values['item_price'] .
                ...
                '</td>
                                <td><i class="fa fa-trash fa-lg text-primary btn" onclick="removeItem(' .
                $values['item_id'] .
                ')"></i></td>
                            </tr>';
            $total = $total + $values['item_quantity'] * $values['item_price'];
...
if (isset($_POST['plus_id'])) {
    foreach ($_SESSION['shopping_cart'] as $keys => $values) {
        if ($values['item_id'] == $_POST['plus_id']) {
            ...
            echo '1 ' .
                $_SESSION['shopping_cart'][$keys]['item_measure'] .
                ' of ' .
                $_SESSION['shopping_cart'][$keys]['item_name'] .
                ' added!';
        }
    }
}
